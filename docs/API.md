...
| Recyclarr | none published | keys in `config/recyclarr/secrets.yml` (generated by `setup.sh`) | — | calls Radarr/Sonarr APIs itself |
...
- Auth: `X-Api-Key`, with keys resolved from `config/recyclarr/secrets.yml` via
  recyclarr's `!secret` substitution (file generated by `scripts/setup.sh`).
- Upstream: <https://recyclarr.dev/>
